...
(g||0!==user_uin&&Math.floor(user_uin/100)%7==q)&&(e.domain_list=L),g&&(e.html_content=m),
...
function i(){
for(var e=window.pageYOffset||document.documentElement.scrollTop,t=r.video_top.length,o=e+r.innerHeight,d=0,c=0;t>c;c++){
var s=r.video_top[c];
s.reported?d++:o>=s.start&&o<=s.end&&(s.reported=!0,n.report({
step:1,
vid:s.vid
}));
}
d==t&&(m.off(window,"scroll",i),r.video_top=r.video_iframe=i=null);
}
e("biz_common/utils/string/html.js");
{
...
video_iframe:[],
video_top:[]
},d=e("pages/version4video.js"),c=e("biz_common/dom/attr.js"),m=(c.setProperty,e("biz_common/dom/event.js")),s=document.getElementsByTagName("iframe");
/MicroMessenger/.test(navigator.userAgent);
}
window.reportVid=[];
for(var a=Math.ceil(1e4*Math.random()),p=0,l=s.length;l>p;++p){
o=s[p];
...
h=h.replace(/&amp;/g,"&");
for(var g=h.split("&"),_=["/mp/newappmsgvote?action=show"],p=0;p<g.length;p++)(0==g[p].indexOf("__biz=")||0==g[p].indexOf("supervoteid="))&&_.push(g[p]);
_.length>1&&(v=_.join("&")+"#wechat_redirect");
}
}
...
dom:o,
vid:x
}),u=["/mp/videoplayer?video_h=",b,"&scene=",window.source,"&random_num=",a,"&article_title=",encodeURIComponent(window.msg_title.htmlDecode()),"&source=4&vid=",x,"&mid=",appmsgid,"&idx=",itemidx||idx,"&__biz=",biz,"&uin=",uin,"&key=",key,"&pass_ticket=",pass_ticket,"&version=",version,"&devicetype=",window.devicetype||"","&wxtoken=",window.wxtoken||""].join(""),
